...
fn data_json_line(env: &Envelope) -> Result<String> {
    ...
    for (key, value) in entries {
        ...
        if key == "body" {
            out.insert(key.to_string(), body_to_json(value));
        } else {
...
fn body_to_json(value: &Value) -> serde_json::Value {
    match value {
        Value::Binary(bytes) => {
            let mut out = Map::new();
...
mod tests {
    ...
    fn renders_binary_data_as_jsonl() {
        ...
        let line = data_json_line(&env).unwrap();
