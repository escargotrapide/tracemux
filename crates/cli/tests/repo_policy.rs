...
fn ai_verify_scripts_emit_required_report_contract() {
    ...
    for text in [&ps, &sh] {
        assert!(text.contains("tracemux/ai-verify/v1"));
...
// REQ: NFR-PORT-001
#[test]
fn tauri_shell_uses_the_single_tracemux_sidecar_binary() {
    ...
    assert!(cli_manifest.contains("name = \"tracemux\""));
    assert!(tauri_conf.contains("binaries/tracemux"));
    assert!(tauri_lib.contains("sidecar(\"binaries/tracemux\")"));
